...
export class If implements Instruction {
  ...
  constructor(
    ...
    listInstructionsElse: Instruction[] | undefined,
    row: number,
    column: number,
    countTab: number
  ) {
  ...
  transpiler(): string {
    ...
    bodyIf += this.listInstructions
      .map((instruction: Instruction) => `${tabs}${instruction.transpiler()}`)
      .join("");

    if (this.listInstructionsElse) {
      bodyElse += this.listInstructionsElse
        .map((instruction: Instruction) => `${tabs}${instruction.transpiler()}`)
        .join("");
    }

    return `if (${condition}) {\n${bodyIf}${tabs.slice(0, tabs.length - 1)}}${
      this.listInstructionsElse
  ...
  public getElseInstructions(): Instruction[] | undefined {
    return this.listInstructionsElse;
  }
}